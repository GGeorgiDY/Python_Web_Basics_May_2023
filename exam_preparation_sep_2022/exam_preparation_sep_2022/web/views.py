...
from exam_preparation_sep_2022.web.forms import ProfileCreateForm, AlbumCreateForm, AlbumEditForm, AlbumDeleteForm, \
    ProfileDeleteForm
...
def index(request):
    ...
    if profile is None:   # Ако нямаме профил
        return redirect('add profile')   # искаме да редиректнеш към 'add profile' view-то
...
def add_album(request):
    if request.method == 'GET':
        form = AlbumCreateForm()    # Създай празна форма - формата за да я върнем.
    else:
...
def details_profile(request):
    ...
    albums_counter = Album.objects.count()
    context = {
        'profile': profile,
